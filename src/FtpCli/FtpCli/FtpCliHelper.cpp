...
bool CFtpOperate::set(CString FullURL,CString UserName,CString Password)
{
    ...
    if(m_strFullURL == _T("") || m_strUserName == _T(""))
	{
	...
	{
		if (m_strFullURL.Left(6) != _T("ftp://"))
		{
			m_strFullURL = _T("ftp://") + m_strFullURL;
		}
		flag = 1;
		return 1;
	}
...
bool CFtpOperate::OpenConnection()
{
	...
	if (!AfxParseURL(m_strFullURL, dwServiceType, m_strServerName, strAppName, nPort))
	{
        ...
        strTime += _T("无法解析 FTP服务器 ") + m_strFullURL +_T("\r\n");
	...
	try 
	{
        ...
        strTime += _T("准备连接FTP服务器 ") + m_strServerName + _T("........\r\n");
        ...
        strTime += _T("成功连接FTP服务器 ") + m_strServerName + _T("\r\n                    当前目录为：") + m_strFullPath + _T("\r\n");
		nLength  =  pEdiMes->SendMessage(WM_GETTEXTLENGTH);  
        pEdiMes->SetSel(nLength,  nLength);  
        pEdiMes->ReplaceSel(strTime); 
		flag = 2; // 连接成功后设为2 
...
bool CFtpOperate::CloseConnection()
{
 ...
 int CFtpOperate::GetFtpFile(CString remote,CString local)						
 {
	 ...
	 {
		 // 二进制方式
		 goodfile = pFtpConnection->GetFile(m_strFullPath+_T('/')+remote,local+remote,
	 ...
	 else
	 {
		 ...
		 strTime += _T("下载文件成功，保存于：") + local+remote + _T("\r\n");
		 nLength  =  pEdiMes->SendMessage(WM_GETTEXTLENGTH);  
		 pEdiMes->SetSel(nLength,  nLength);  
		 pEdiMes->ReplaceSel(strTime); 
	 }
...
bool CFtpOperate::setNextPath(CString strPath) // 设置服务器下一层文件目录
{
	...
	if (m_strFullPath.GetAt(i) != _T('/'))
	{
		m_strFullPath += _T('/');
	}
	m_strFullPath += strPath + _T('/');
	CurrentTime = CTime::GetCurrentTime();
	strTime.Format(_T("%d/%d/%d %d:%d:%d   "), CurrentTime.GetYear(), CurrentTime.GetMonth(),
		CurrentTime.GetDay(), CurrentTime.GetHour(), CurrentTime.GetMinute(),
		CurrentTime.GetSecond());

	strTime += _T("当前目录为：") + m_strFullPath + _T("\r\n");
	nLength = pEdiMes->SendMessage(WM_GETTEXTLENGTH);
	pEdiMes->SetSel(nLength, nLength);
	pEdiMes->ReplaceSel(strTime);
	return 1;
...
int CFtpOperate::DeleteFTPf(CString remote,int index)  
{
	if (flag != 2)
	 ...
	 CurrentTime=CTime::GetCurrentTime(); 
		...
		if (index == 0)
		{
			strTime += _T("删除文件夹，请稍候： ") + remote + _T(".......\r\n");
		}
		else
		{
			strTime += _T("删除文件，请稍候： ") + remote + _T(".......\r\n");
		}
	 ...
	 {
		 // 删除文件夹
		 goodfile = pFtpConnection->RemoveDirectory(m_strFullPath+_T('/')+remote);
	 }
	 else
	 {
		 // 删除文件
		 goodfile = pFtpConnection->Remove(m_strFullPath+_T('/')+remote);
	 }
