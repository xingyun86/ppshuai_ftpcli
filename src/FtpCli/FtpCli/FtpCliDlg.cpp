...
BOOL CFtpDlg::OnInitDialog()
{
	...
	m_editPort.SetWindowText(_T("21"));
	bBinary1 = 1; // 使用二进制
	bPassive1 = 0; // 使用主动
	myPort = 21;
	...
	m_editServer.SetWindowText(_T("ftp://ftp1.linuxidc.com"));
	m_editName.SetWindowText(_T("ftp1.linuxidc.com"));
	m_editPassword.SetWindowText(_T("www.linuxidc.com"));
	...
	m_ctrlLocalFile.InsertColumn(0, _T("文件名"), LVCFMT_LEFT, 140, 0);//设置列
	m_ctrlLocalFile.InsertColumn(1, _T("文件类型"), LVCFMT_LEFT, 100, 1);
	m_ctrlLocalFile.InsertColumn(2, _T("修改日期"), LVCFMT_LEFT, 100, 2);
	m_ctrlServerFile.InsertColumn(0, _T("文件名"), LVCFMT_LEFT, 140, 0);//设置列
	m_ctrlServerFile.InsertColumn(1, _T("文件类型"), LVCFMT_LEFT, 100, 1);
	m_ctrlServerFile.InsertColumn(2, _T("修改日期"), LVCFMT_LEFT, 100, 2);

	// 添加硬盘盘符
	DWORD dwDriveIndex = 0L;
	_TCHAR tzDriveStrings[MAXCHAR] = { 0 };
	DWORD dwDriveStringsSize = ::GetLogicalDriveStrings(sizeof(tzDriveStrings) / sizeof(*tzDriveStrings), tzDriveStrings);
	for (dwDriveIndex = 0L; dwDriveIndex < dwDriveStringsSize; dwDriveIndex += lstrlen(tzDriveStrings) + 1)
	{
		m_ctrlLocal.AddString(tzDriveStrings + dwDriveIndex);
	}

	int index = m_ctrlLocal.FindString(0, _T("c:\\"));
	m_ctrlLocal.SetCurSel(index);
	m_strLocPath = _T("c:\\"); // 初始化为c盘
	ListLocFile();
...
void CFtpDlg::OnBtnConnect() 
{
	...
	if (i)
	{
        ...
        if (cFtpOpr.OpenConnection())
		{
			...
			m_ctrlServer.AddString((LPCTSTR)temp);
...
void CFtpDlg::OnDblclkListLocalfile(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (SecondColumn==_T("文件夹")) 
	{
		CString strTemp = FirstColumn;
		m_strLocPath += strTemp + _T('\\');
        ListLocFile(); // 更新本地磁盘文件显示
	}
	else if (SecondColumn==_T("文件")) 
	{
		// 点到的是文件，进行上传
...
void  CFtpDlg::ListLocFile()
{
	...
	int k = 0;
	m_ctrlLocalFile.InsertItem(0,_T("...上层目录"));
	LPCTSTR lpPath = (LPCTSTR)m_strLocPath;
	_TCHAR szFind[MAX_PATH] = { 0 };
		...
		while(TRUE)
		{
			if(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				if (str != _T('.') && str != _T(".."))
				{
					...
					m_ctrlLocalFile.SetItemText(k+1,1,_T("文件夹"));
					++k;
				}
			...
			else
			{
				...
				m_ctrlLocalFile.SetItemText(k+1,1,_T("文件"));
...
void CFtpDlg::OnBtnup() 
{
	...
	if (SecondColumn==_T("文件夹")) 
	{
		...
		CString strTime;
		strTime.Format(_T("%d/%d/%d %d:%d:%d   "),CurrentTime.GetYear(),CurrentTime.GetMonth(),
			CurrentTime.GetDay(),CurrentTime.GetHour(),  CurrentTime.GetMinute(),
			CurrentTime.GetSecond());
		
        strTime += _T("暂不支持上传文件夹!\r\n");
...
void CFtpDlg::UpdateServerFile()// 更新服务器的文件显示
{
    ...
    for(int test=0;test<nFileNumber;test++)
	{
		...
		if (stTemp.Right(5) == _T("<DIR>"))
		{
			...
			m_ctrlServerFile.SetItemText(test+1,1,_T("文件夹"));
		}
		else
		{
					 // 文件
			...
			lvitem.mask = LVIF_IMAGE|LVIF_TEXT;   
			lvitem.iItem =test+1;   
			lvitem.pszText = (LPTSTR)(LPCTSTR)stTemp;   
			lvitem.iImage = 1;   
			lvitem.iSubItem =0;   
			m_ctrlServerFile.InsertItem(&lvitem); 
			m_ctrlServerFile.SetItemText(test+1,1,_T("文件"));
		}
	}
...
void CFtpDlg::OnChkanonymous() 
{
	...
	if (m_chkAnony.GetCheck())
	{
        ...
        m_editName.SetWindowText(_T("anonymous"));
